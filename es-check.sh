...
cluster_diskusage(){
cluster_nodes_all_disk_total=$(curl -s -u elastic:abc123321 '10.113.12.20:9200/_cat/nodes?v&h=diskTotal' | awk ' {sum += $1};END {print sum}' )
...
case ${#document_count} in
    ...
    ;;
    2)  echo -e "当前集群文档总数是：$document_count" | tee -a "$ESLOG"
    ;;
    3)  echo -e "当前集群文档总数是：$document_count" | tee -a "$ESLOG"
    ;;
    4)  echo -e "当前集群文档总数是：${document_count:0:1},${document_count:1}" | tee -a "$ESLOG"
    ;;
    5)  echo -e "当前集群文档总数是：${document_count:0:2},${document_count:2}" | tee -a "$ESLOG"
    ;;
    6)  echo -e "当前集群文档总数是：${document_count:0:3},${document_count:3}" | tee -a "$ESLOG"
    ;;  
    7)  echo -e "当前集群文档总数是：${document_count:0:1},${document_count:1:3},${document_count:4}" | tee -a "$ESLOG"
    ;;
    8)  echo -e "当前集群文档总数是：${document_count:0:2},${document_count:2:3},${document_count:5}" | tee -a "$ESLOG"
    ;;
    9)  echo -e "当前集群文档总数是：${document_count:0:3},${document_count:3:3},${document_count:6}" | tee -a "$ESLOG"
    ;;
